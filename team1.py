...
#  구글 드라이브 API 설정 정보
# ──────────────────────────────────────────
# 2단계에서 다운로드한 서비스 계정 키 파일 이름
CREDENTIALS_FILE = "credentials.json" 

# 3단계에서 복사한 구글 드라이브 data.json 파일의 고유 ID (문자열로 입력)
DRIVE_FILE_ID = "1hE3eLy7RoairKHQljanQ65SdswQEOwSx" 

def get_drive_service():
    """구글 드라이브 API 서비스 객체를 생성하여 반환합니다."""
    SCOPES = ['https://www.googleapis.com/auth/drive']
    
    # 인증 파일(credentials.json)을 읽어 인증 자격 증명 생성
    creds = service_account.Credentials.from_service_account_file(
        CREDENTIALS_FILE, scopes=SCOPES
    )
    # 구글 드라이브 API 서비스 빌드 (v3 버전 사용)
    return build('drive', 'v3', credentials=creds)


# ──────────────────────────────────────────
#  데이터 불러오기 (구글 드라이브에서 다운로드)
# ──────────────────────────────────────────
def load_data():
    try:
        service = get_drive_service()
        
        # 구글 드라이브에 파일 내용을 보내달라고 요청 (get_media)
        request = service.files().get_media(fileId=DRIVE_FILE_ID)
        
        # 다운로드할 데이터를 담을 가상 메모리 버퍼(BytesIO) 생성
        fh = io.BytesIO()
        downloader = MediaIoBaseDownload(fh, request)
        
        # 데이터를 청크(조각) 단위로 끝까지 다운로드
        done = False
        while not done:
            status, done = downloader.next_chunk()
            
        # 다운로드된 바이트 데이터를 UTF-8로 디코딩한 후, JSON(딕셔너리)으로 변환
        return json.loads(fh.getvalue().decode('utf-8'))
        
    except Exception as e:
        # 파일이 비어있거나 에러가 발생하면 빈 딕셔너리 반환
        print(f"⚠️ 구글 드라이브 데이터 로드 실패: {e}")
        return {}


# ──────────────────────────────────────────
#  데이터 저장하기 (구글 드라이브 파일 덮어쓰기)
# ──────────────────────────────────────────
def save_data(data):
    try:
        service = get_drive_service()
        
        # 1. 파이썬 딕셔너리를 가독성 좋은 JSON 문자열로 변환
        json_string = json.dumps(data, ensure_ascii=False, indent=2)
        
        # 2. 문자열을 바이트 데이터로 바꾸어 가상 메모리 버퍼에 저장
        fh = io.BytesIO(json_string.encode('utf-8'))
        
        # 3. 업로드할 파일의 타입(JSON)과 미디어 객체 지정
        ...
        # 4. 기존 파일 ID(DRIVE_FILE_ID)를 지정하여 클라우드 상의 파일 내용을 업데이트(덮어쓰기)
        service.files().update(
            fileId=DRIVE_FILE_ID, 
            media_body=media
        ).execute()
        
        print("✅ 구글 드라이브에 데이터가 성공적으로 동기화되었습니다.")
        
    except Exception as e:
        print(f"❌ 구글 드라이브 데이터 저장 실패: {e}")
        messagebox.showerror("오류", f"데이터를 클라우드에 저장하지 못했습니다.\n{e}")
print("로그인 성공!")
